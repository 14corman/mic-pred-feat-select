...
class DenseNN:
    def __init__(self, properties, antibiotic_name, num_genes, classes, batch_size=32):
        self.num_folds = 5
    ...
    def build_model(self, columns):
        ...
        def func():
            ...
            previous_layer = all_inputs

            while len(previous_layer) > 30:
                index = 0
                tmp_layer = []
                next_layer = []
                for i in range(len(previous_layer)):
                    if index < 2:
                        tmp_layer.append(previous_layer[i])
                        index += 1
                    else:
                        tmp_layer.append(previous_layer[i])
                        all_features = tf.keras.layers.concatenate(tmp_layer)
                        x = tf.keras.layers.Dense(10, activation="sigmoid")(all_features)
                        next_layer.append(x)
                        tmp_layer = []
                        index = 0
            ...
            all_features = tf.keras.layers.concatenate(previous_layer)
            x = tf.keras.layers.Dense(30, activation="sigmoid")(all_features)
            x = tf.keras.layers.Dense(20, activation="sigmoid")(x)
            # x = tf.keras.layers.Dense(500)(all_features)
            ...
            model = keras.Sequential(
                [
                    layers.Dense(2000, activation="relu", input_shape=(num_genes[gene_id],)),
    ...
    def train(self, train_data, train_labels, combined_labels):
        ...
        for gene_id, gene in enumerate(["ompk35", "ompk36", "ompk37"]):
            ...
            input_dataset.to_csv(f"{self.properties.data_dir}{self.properties.processed_data}combined_{gene}_data_processed.csv")
            tmp_dataset = train_data[gene_id].join(train_labels[gene_id], how="inner")
            tmp_dataset.to_csv(
                f"{self.properties.data_dir}{self.properties.processed_data}combined_{gene}_data.csv")

            train_nn_data, val_nn_data = train_test_split(input_dataset, test_size=0.2)
            ...
            history = self.models[gene_id].fit(train_nn_dataset, epochs=20, validation_data=val_nn_dataset)
            tf.keras.utils.plot_model(self.models[gene_id], show_shapes=True, rankdir="LR",
                                      to_file=f'{self.properties.output_dir}models/nn_dense_{gene}_{self.antibiotic_name}.png')
            self.models[gene_id].save(f'{self.properties.output_dir}models/nn_dense_{self.antibiotic_name}.model')
            ...
            predictions = self.models[gene_id].predict(prediction_input)
        ...
        #     Path(f"{self.properties.output_dir}nn_dense_nn_data_report.html"))
        ...
        tf.keras.utils.plot_model(self.output_model, show_shapes=True, rankdir="LR",
                                  to_file=f'{self.properties.output_dir}models/nn_dense_out_nn_dense_{self.antibiotic_name}.png')
        history = self.output_model.fit(train_nn_dataset, epochs=20, validation_data=val_nn_dataset)
        self.output_model.save(
            f'{self.properties.output_dir}models/nn_dense_out_nn_dense_{self.antibiotic_name}.model')
